...
### Model and Conclusions {data-commentary-width=600}

```{r}
tabsetPanel(type="tabs",
    tabPanel("Model Overview", 
    
tags$div(tags$i(tags$strong("Click through the small tabs"))," on this model storyboard to see how various model types perform in predicting cancer rates (cancer_per100k) and the varying importance of the predictors in producing that model fit. Multiple models were considered for this project including decision tree, random forest, and gradient-boosted trees. Since there were not any strong relationships in the exploration storyboard, further linear model fitting were replaced with various tree methods including decision tree, random forest, and gradient boosting models. As this is a study with potential public health interest, models with interpretability are important so the factors affecting cancer rates can be identified. Therefore models low interpretability were not considered.",tags$br(),tags$br(),
         "The response variable is the age-adjusted cancer rate per 100k people (cancer_per100k).",tags$br(),tags$br(),
         "All models are using 11 variables as predictors and are numeric unless otherwise indicated.",tags$ul(
...
tabPanel("Decision Tree Model",
   ...
   fillRow(
     ...
     tags$div(withTags(p("The decision tree model is shown on the main graph to the left.",i(strong("Hover, click, and pan"))," around the tree to see the most important nodes. ",i(strong("Hover over"))," the terminal nodes (purple boxes) to see stats on that result, and ",i(strong("click"))," rules to see the criteria for that terminal node. The cancer rates generally go from low to high as you go from the left to the right on the terminal nodes of the tree. There are two different trees that are considered, a full tree model with all 11 predictors and an 8 predictor model without any location information (state, region, epa_region). ", i(strong("Select the tree type below"))," to see each tree type.")),
...
p("Each Cp value for the trees was set to 0.03. The results for the full tree shows that state is clearly the strongest predictor, occupying many nodes of the tree. Other variables that come up are number of renewable powerplants, generating capacity for carbon plants, and the summed Superfund site score. These nodes deeper in the tree means that environmental effects are only important for certain states. For example, hovering over the pp_renewable node shows that having 3 or more renewable plants in AZ, CO, HI, NM, and NV raises cancer rate. All of the non-state variables see increases in the variable associated with higher cancer rates. It is good to see that generating capacity for carbon increases cancer rates over other types of energy generation.",br(),br(),
...
tabPanel("Random Forest Model",
          fillRow(plotlyOutput("forest_mse"),
                  ...
                  withTags(p("Random forest of various hyperparameters were considered on the plotly graph on the far left. 5-fold cross validation test mean-squared error (MSE) is used to compare the different models. ",i(strong("Zoom and hover")), " around the plotly graph to see the results of the hyperparameter optimization.",br(),br(),"At ~100 trees the CV test MSE levels out and no further test MSE improvement occurs. The chosen 'best' model is marked with a red triangle at ntree=100, mtry=4 with a test MSE = 2305.1. There is no evidence that overfitting has occurred as the test MSE does not rise despite increasing tree numbers. The center graph shows predictors on the y axis, and the colors show the distribution of depth in the tree. Predictors that are more important will be closer to the root of the trees of the random forest. We see that state is the strongest indicator, with an average depth in the forest of 0.92. The colors on the state bar show the distribution of node locations among the 100 trees. The top three variables are the location factor variables state, epa_region, and region. This shows that there are significant regional effects on cancer rates.",br(),br(), "The second group of factors include a combination of environmental factors. Energy capacity (MW) shows up multiple times for sub-categories and for all powerplants. Summed Superfund site score also appears, showing that contamination has a role to play in cancer rates."))
...
tabPanel("Gradient Boosting Model",
             fillRow(plotlyOutput("gbm_mse"),
                     ...
                     withTags(p("Model fit using gradient boosted trees yielded slightly improved results from the random forest models. Hyperparameter grid optimization was performed to minimize 5-fold cross validated test mean-squared error (MSE).",i(strong("Zoom and hover")), " around the plotly graph on the far left to examine the results.",br(),br(),
    "The best hyperparameter combination is found at the red triangle. The learning rate (shrinkage) becomes too quick at shrinkage=0.01, so the best shrinkage rate is 0.0075. The interaction depth shows that larger depths have lower test MSEs more quickly, but that the larger depth overfit with the addition of too many trees. This leads to an optimal solution of trees = 350 and dpeth = 6. Trees up to 2500 were considered and no further improvement happened after tree=350. The best test MSE for the GBM is 2189.6",br(),br(),
    
    "The variable importance for the best GBM model shows that the state variable takes on an outsize importance in explaining cancer rate variation. The next three most important predictors are electricity capacity variables. Sum site score and the number of powerplants are minor contributors to this model."
...
tabPanel("Conclusions",
         withTags(p("In conclusion, this study finds that there is large variation in the cancer rates for counties across the United States. While there are many potential causes for cancer, environmental factors play some role but are not the only reason that some counties have particularly higher cancer rates than others. When examining the stark difference in cancer rates between counties in Eastern Kentucky and Southwest Virginia, the models point not to environmental causes such as pollution from powerplants or environmental contamination from Superfund sites as the primary cause, but to state-specific factors. Just being in Kentucky means that cancer rates are going to be higher than almost anywhere else in the country. This was seen in exploratory analysis as well. All of the decision tree models concurred that state residence is the most important factor for cancer rates by county. Energy production does play a role in cancer rates variation, and that carbon-based powerplants that are worse polluters generally seem to have more of an effect than renewable powerplants. Further research on this topic is necessary and would include the addition of more predictors for cancer including healthcare information, morbidities from other poor health diseases such as heart disease, as well as specific water and air pollution data.",br(),br(),"Please see the appendix storyboard to see the raw data used in this project and brief data methods information.")))
)
...
output$tree_model <- renderUI({
  ...
  if (tree_choice == "tree_simple"){
    tree_url = 'https://andersonmonken.georgetown.domains/wp-content/uploads/2020/05/full_tree.html'
    
  }
  else if (tree_choice == "tree_nolocation"){
    tree_url = 'https://andersonmonken.georgetown.domains/wp-content/uploads/2020/05/nolocation_tree.html'
    
  }
  tags$iframe(src=tree_url, width = '100%', height = '900px', style="border: none") #
...
output$forest_importance <- renderPlot({readRDS('forest_importance_plot.rds') + ggtitle("Best Random Forest Model (mtry=4, ntree=100): Distribution of minimal depth and its mean")},height = 650)
...
### Intro: Heatmap of Cancer Rates by County {data-commentary-width=400}
...
This project investigates factors affecting county-level cancer rates around the United States. There are many factors that contribute to cancer, including biological, occupational risk, lifestyle, and environmental causes. This story will focus on how environmental contamination from toxic waste sites and powerplants have an effect on cancer rates. 

Start off by ***clicking*** on a few counties on the heatmap to get yourself familiar with the NIH data for age-adjusted cancer rates. In subsequent frames more variables will be introduced.

Let’s examine the distribution of cancer rates around the country. ***Click the buttons below*** to see the location of the 25 highest and 25 lowest cancer rate counties on the map. ***Click on the info buttons*** to show info for the highest or lowest counties in the US.
...
***Change*** the # below to show greater or fewer counties in the rankings, then ***click*** "Reset Map", and ***click*** the highest/lowest buttons to see the # ranking. You must reset the map for number selection to be refelcted when clicking on the ranking.
...
The West and Southwest have many of the counties with the lowest cancer rates. Many of the higher cancer rate counties are in the South, particularly in Kentucky. The most interesting border between the best and worst counties appears near the tri-cities region in southwest Virginia. Kentucky counties are experiencing the highest cancer rates, while their Virginian neighbors have the lowest rates in the country. Many counties in the West and Southwest have the lowest rates of cancer in the county.

***Click on the arrows*** in the top left to navigate through the storyboards for this project.
...
output$plotly_cancer_dist <- renderPlotly({
  ...
  plot_ly(
        ...
        pointpos = -2,
        #hoverinfo = "text",
                     text = paste0("<b>County: </b>", map_data_df_dist$county,", ",map_data$state,
...
As the heatmap on the previous board showed, there are clear differences in cancer rates across difference parts of the United States. Examine these differences further by ***selecting*** the administrative breakdown type of your choice. For Census Regions, distribution of cancer rates shows the West having the lowest rates. ***Hover*** your mouse over the dots to see the individual county info and the box plot to get the quantile values.
...
But what about if we exclude some of the worst or best states from the groups? In particular there was a stark difference between counties on the border between Kentucky and Virginia. 

```{r}
#tags$style(type='text/css', ".selectize-dropdown-content {max-height: 200px; }")
pickerInput("exclude_state_dist_us","Exclude U.S. States",choices = c('AL','AK','AZ','AR','CA','CO','CT','DC','DE','FL','GA','HI','ID','IL','IN','IA','KY','LA','ME','MD','MA','MI','MS','MO','MT','NE','NH','NJ','NM','NV','NY','NC','ND','OH','OK','OR','PA','RI','SC','SD','TN','TX','UT','VA','VT','WA','WV','WI','WY'), options = list(`actions-box` = TRUE,`size` = 8),multiple = T)
```

Try ***selecting*** Kentucky on the state exclusion list to see how removing those counties affects the distributions. Try ***selecting more than one*** to see the effects. ***Flip*** back and forth between this and the previous board to pick out high or low cancer rate states and see the effecto of removing them from the box plots.
...
Superfund sites are locations of environmental contamination that the Environmental Protection Agency (EPA) has stepped in to perform long-term cleanup of the area at the expense of the offending group and increasingly taxpayers. The site score increases 0 to 100 as the degree of hazard increases. Sites are removed from the list that are cleaned up. Sites have to be added to the list, and sometimes that can take a while. Only sites that were added to the list before July, 2014 are included on this map. The site scores are summed to the county level to show the broad environmental hazard for that county.

On the upper right control panel of the map, ***choose*** among different heatmap options to visually explore the relationship between superfund site scores, number of sites, and cancer rates. You can also use the ***checkbox*** to show/hide the the location of individual superfund sites. ***Zoom in/out and click*** around the map to see information for individual superfund sites or counties. Counties that are grey are the counties are missing the cancer rate variable. 
...
### Powerplants by Energy Production Type
...
***
Just like the last storyboard,  ***choose*** among different heatmap options to visually explore the relationship between energy production and cancer rates. Use the ***checkbox*** to show/hide the powerplant markers. ***Zoom in/out and click*** to see stats on energy generation at individual plants or counties. In particular take a look at Southern California and Kentucky. Flip back and forth between theatmaps to see where there are large differences.

***Select*** the powerplant type then ***click*** the "Reset Map" button to see that segment of the energy sector on the map. Two aggregations are set up for carbon-based powerplants (petcoke, coal, oil and natural gas) and renewable powerplants (biomass, geothermal, hydro, nuclear, solar, wind).

```{r}
selectizeInput("choose_plant_type","Choose Powerplant Type",choices = list('All','Carbon','Renewable',
...
### Further Explore Relationships to Cancer Rates {data-commentary-width=360}

```{r, eval = TRUE}
fillRow(leafletOutput("cancer_map"),plotlyOutput("cancer_plotly"), flex = c(2,1))
...
output$cancer_plotly <- renderPlotly({
  ...
  layout(showlegend = TRUE,
        title = list(text = paste0("Relationship between ",var_y," and ",var_x)),
...
Now it's time to explore for you to explore the potential relationships between cancer rates and environmental factors. ***Zoom and pan*** around the map to decide the region you want to perform analysis on. Notice that the scatterplot will adjust based on your map movement. ***Click on a county on the map*** to see its position on the scatterplot. ***Click a point on the scatterplot*** to see info about it displayed on the map.

***Choose your X and Y Variables*** on the righthand panel and ***click*** "Reset Variables for Analysis" to update the scatterplot. ***Choose*** your regression line and ***adjust*** the degrees of freedom (for non-lm reg.).

The quick conclusions from this board shows that there are slightly positive relationships between summed site score and cancer per 100k. Many of the relationships are relatively weak.
...
observeEvent(input$cancer_map_shape_click, { 
  ...
  print(paste0('FIPS ID: ',input$cancer_map_shape_click$id,', x var: ',df_cancer_sub[[cancer_var_x]],', y var: ',df_cancer_sub[[cancer_var_y]]))
...
observeEvent({event_data("plotly_click", source="cancer_plotly")}, {
    ...
    message(paste("Plotly point clicked:",shape_i$county,shape_i$state))
    message(click_data)
...
observeEvent(c(input$reg_line,
  ...
  if (input$reg_line == 'lm'){
    ...
    fit <- lm(df_plot[[var_y]] ~ poly(df_plot[[var_x]],input$fit_df))
    fit_name <- paste0(input$reg_line, ' fit, df:',input$fit_df)
  }
  else if (input$reg_line == 'spline'){
    fit <- lm(df_plot[[var_y]] ~ s(df_plot[[var_x]],input$fit_df))
    fit_name <- paste0(input$reg_line, ' fit, df:',input$fit_df)
  }  
  else if (input$reg_line == 'natural-spline'){
    fit <- lm(df_plot[[var_y]] ~ ns(df_plot[[var_x]],input$fit_df))
    fit_name <- paste0(input$reg_line, ' fit, df:',input$fit_df)
  }  
  else if (input$reg_line == 'basis-spline'){
    fit <- lm(df_plot[[var_y]] ~ bs(df_plot[[var_x]],input$fit_df))
    fit_name <- paste0(input$reg_line, ' fit, df:',input$fit_df)
  }  
...
selectInput("cancer_var_x","Select X Variable",list(            "# Superfund Sites" = "num_sites",
...
selectInput("cancer_var_y","Select Y Variable",list("Cancer Rate/100k" = "cancer_per100k",
                                               "5-yr Cancer Trend" = "trend5yr",
                                               "Avg. Annual Cancer Count" = "avg_annual_count"
                                               ), selected = "cancer_per100k")

selectInput("reg_line", "Add Regression Line", list("None" = "",
                                           "Linear Regression" = "lm",
                                           "Polynomial" = "polynomial",
                                           "Smoothing Spline" = "spline",
                                           "Natural Spline" = "natural-spline",
                                           "Basis Spline" = "basis-spline"), selected = '')

sliderInput("fit_df","Degrees of Freedom", value = 2, min = 1, max = 10, width = '80%')

fluidRow(actionButton("change_vars_button", "Reset Variables for Analysis"), align = "center")
fluidRow(actionButton("clear_fits", "Clear Fits"),actionButton("clear_cancer_popup_button", "Clear popups on map"), align = 'center')
```
...
***Choose*** the data source from the list below and ***explore*** the raw data on the main panel.

```{r}
#p("By default DT allows multiple row selection. Selected rows are...")
#renderPrint(input$irisTable_rows_selected)
selectInput("data_select", NULL, choices = list("County-level Data" = "county",
                                         "Powerplant Data" = "pp",
