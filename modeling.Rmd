...
visTree(tree_models[[tree_choice]], main = tree_title, height = '800px')
...
gbm_importance <- ggplot(summary_gbm_importance %>% filter(rel.inf > 0.05), aes(x = reorder(var, rel.inf),y=rel.inf)) + geom_col(fill = 'darkblue') + coord_flip() + ylab("Relative Importance") + xlab("Variable") + ggtitle("Variable Importance for Best GBM\n (ntree = 350, depth = 6, shrinkage = 0.01)")
